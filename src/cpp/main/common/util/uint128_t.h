...
class uint128_t{
    ...
    public:
        ...
        template <typename T> uint128_t operator%(T rhs){
            // ez csak a sajat balrolszorzassal megy
            return *this - (rhs * (*this / rhs));
            // jobbrolszorzassal eredetileg is menne
...
// sajat probalkozas a fenti helyett
template <typename T> uint128_t operator*(T lhs, uint128_t rhs){
    return rhs * uint128_t(lhs);
}
...
// IO Operator
/*std::ostream & uint128_t::operator<<(std::ostream & stream, uint128_t rhs){
    std::string out = "";
    if (rhs == 0)
        out = "0";
    else {
