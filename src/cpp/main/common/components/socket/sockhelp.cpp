...
int atoport(char *service, const char* proto)
{
  int port;
  long int lport;
  struct servent *serv;
...
struct in_addr *atoaddr(char *address)
{
  ...
  if (static_cast<int>(saddr.s_addr) != -1) {
    return &saddr;
  }
...
int make_connection(int port, int socketType, char *netaddress)
{
  char service[10];
  sprintf(service, "%d", port);
  fprintf(stderr, "make_connection port=%s\n", service);
  return make_connection(service, socketType, netaddress);
...
int sock_gets(int sockfd, char *str, size_t count)
{
  ...
  fprintf(stderr, "---2\n");
    ...
    if ( (total_count < static_cast<int>(count)) && (last_read != 10) && (last_read !=13) ) {
...
int sock_puts(int sockfd, const char *str, int len)
{
  return sock_write(sockfd, str, len);
}
