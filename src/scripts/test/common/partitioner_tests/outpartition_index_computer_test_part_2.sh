...
inputPartition="$resourceDir"/split/slavery_2.txt
config="$resourceDir"/slavery.cfg

outNeighbors="$work_dir"/nn.txt
outPartitionIndices="$work_dir"/opi.txt

expectedNeghbors=$resourceDir/expected_partition_indices/nn_2.txt
expectedOutPartitionIndices=$resourceDir/expected_partition_indices/opi_2.txt

"$this_dir"/../../../../bin/main/common/tools/outpartition_index_computer_job \
"$inputPartition" "$config" 4 10000000 2 $outNeighbors $outPartitionIndices
